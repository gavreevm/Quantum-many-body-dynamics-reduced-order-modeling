...
for i, (key, value) in enumerate(sorted(data.items(), key=lambda kv: kv[0])):
    ...
    plt.xticks([0, 4, 9, 14, 19, 24], latex([1, 5, 10, 15, 20, 25]), fontsize=16)
    if (i % 2 == 0):
        plt.yticks([i * (N / 3) for i in range(4)], latex([round(tau * i * (N / 3), 2) for i in range(4)]), fontsize=16)
...
plt.savefig('plots_experiment1/mutual_information_flows.svg')

plt.figure(figsize=(10, 7.5))
plt.subplots_adjust(hspace = 0.2, wspace = 0.2)
ax = plt.subplot(2, 2, 1)
ax.set_title(r'${1-{\rm st \ spin \ dynamics}}$', fontsize=18)
...
ax.set_title(r'${14-{\rm th \ spin \ dynamics}}$', fontsize=18)
plt.ylim(top=1.1, bottom=-1.1)
plt.xticks(list(map(lambda x: x / tau, [0, 1, 2, 3, 4, 5, 6, 7])), latex([0, 1, 2, 3, 4, 5, 6, 7]))
...
plt.savefig('plots_experiment1/dynamics.svg')
...
plt.xticks(list(map(lambda x: x / tau, [0, 1, 2, 3, 4, 5, 6, 7])), latex([0, 1, 2, 3, 4, 5, 6, 7]), fontsize=15)
plt.xlabel(r'${\rm Time}$', fontsize=15)
plt.ylim(top = 8e3)
plt.savefig('plots_experiment1/reduced-order_model_dim.svg')
